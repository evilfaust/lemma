...
// Единственное место с человеческими описаниями модификаторов — их читает
// `shared/TableModifiersHelp.jsx` (кнопка «?» рядом с меню «Таблица» в
// редакторе задачи и в теории). Порядок здесь = порядок карточек в справке.
// Синонимы В ЭТОТ список дублировать не нужно — они собираются из
// `MOD_ALIASES` автоматически (`modifierAliases`), поэтому справка не может
// разойтись с тем, что реально понимает парсер.
export const MODIFIER_INFO = {
  plain: {
    title: 'Без линий',
    description: 'Колонки-раскладка без рамок и заливки — для заданий на соответствие («А) … | 1) …»).',
  },
  noheader: {
    title: 'Без шапки',
    description: 'Первая строка перестаёт быть жирным заголовком с заливкой — становится обычной ячейкой.',
  },
  answers: {
    title: 'Бланк ответа',
    description: 'Пустые клетки увеличенной высоты — ученик вписывает ответ от руки.',
  },
  compact: {
    title: 'Компактная',
    description: 'Уменьшенные отступы в ячейках — когда строк много и таблица не помещается.',
  },
  gallery: {
    title: 'Галерея',
    description: 'Сетка чертежей: первая строка — не шапка, все колонки одной ширины. Для «на каком рисунке изображено множество решений».',
  },
  grid: {
    title: 'Обычная сетка',
    description: 'Стандартная таблица с рамками — явно отключает автоподбор, когда таблица похожа на бланк ответа, но это не он.',
  },
  equalcols: {
    title: 'Равные колонки',
    description: 'Столбцы одной ширины, картинка/чертёж внутри растягивается на всю ячейку (пропорции не искажаются). Не даёт графикам в соседних колонках разъехаться по размеру, если у них разный `size`. В отличие от «Галереи» шапку и рамки не трогает — можно сочетать с любым другим модификатором.',
  },
};

/** Ключ модификатора → все его синонимы-директивы (`{…}`), в порядке объявления. */
export function modifierAliases(key) {
  return Object.keys(MOD_ALIASES)
    .filter((alias) => MOD_ALIASES[alias] === key)
    // Однословные и короткие — вперёд, читать список проще.
