...
// Иррациональных границ у систем не бывает, поэтому выбор простой:
// пускать ли дробные границы вообще
const BOUND_KINDS = [
...
const ANSWER_FORMS = [
  { value: 'interval',   label: '(2; 7]' },
  { value: 'inequality', label: '2 < x ⩽ 7' },
];

export default function LinearSystemsGenerator() {
  const {
    title, setTitle,
  ...
  const instruction = linsysInstruction(settings.categories);

  return (
    <>
      <TrigGeneratorLayout
        icon={<MergeCellsOutlined style={{ fontSize: 14 }} />}
        title={title}
        ...
        left={
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 10 }}>
            ...
            {/* Категории заданий */}
            {CATEGORY_GROUPS_LINSYS.map(group => (
              <TrigSettingsSection
                key={group.label}
                label={
                  <TrigBlockToggle
