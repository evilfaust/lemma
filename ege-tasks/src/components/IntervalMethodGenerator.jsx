...
// Промежутками отвечают на ЕГЭ — для метода интервалов это основная запись
const ANSWER_FORMS = [
  { value: 'interval',   label: '(−∞; 2) ∪ (5; ∞)' },
  { value: 'inequality', label: 'x < 2 или x > 5' },
];

export default function IntervalMethodGenerator() {
  const {
    title, setTitle,
  ...
  const instruction = intervalInstruction(settings.categories);
      ...
      <TrigGeneratorLayout
        ...
        left={
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 10 }}>
            ...
            {/* Категории заданий */}
            {CATEGORY_GROUPS_INTERVAL.map(group => (
              <TrigSettingsSection
                key={group.label}
                label={
                  <TrigBlockToggle
              ...
              >
                <CategoryChecklist
                  keys={group.keys}
                  labels={CATEGORY_LABELS_INTERVAL}
                  categories={settings.categories}
