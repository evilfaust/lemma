...
const ANSWER_FORMS = [
  { value: 'interval',   label: '(0; 3]' },
  { value: 'inequality', label: '0 < x ⩽ 3' },
];

export default function QuadraticSystemsGenerator() {
  const {
    title, setTitle,
  ...
  const instruction = qsysInstruction(settings.categories);
  ...
  return (
    <>
      <TrigGeneratorLayout
        ...
        left={
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 10 }}>
            ...
            {/* Категории заданий */}
            {CATEGORY_GROUPS_QSYS.map(group => (
              <TrigSettingsSection
                key={group.label}
                label={
                  <TrigBlockToggle
      ...
      <TrigMCSection
        ...
        tasksData={tasksData}
        generatorType="quadratic_systems"
