...
# Что делает:
#   - Собирает фронтенд
#   - Копирует на VPS: student.html + assets + achievements + иконки/логотип
...
# Копируем корневые статические ресурсы (favicon, логотип, иконка)
rsync -avz \
    "$LOCAL_PROJECT_DIR/ege-tasks/dist/vite.svg" \
    "$LOCAL_PROJECT_DIR/ege-tasks/dist/icon-new.png" \
    "$LOCAL_PROJECT_DIR/ege-tasks/dist/lemma-logo-new.png" \
    "$VPS_HOST:$VPS_FRONTEND_DIR/"
