...
### `OralWorksheetGenerator` (Существующий)

**Назначение:** Генератор "Устный счёт" с распределением по тегам

**Может использовать в будущем:**
- 🔄 `useWorksheetGeneration` - рефакторинг логики генерации
- ✅ `useTaskDragDrop` - уже используется
- 🔄 `useWorksheetActions` - рефакторинг сохранения

---

### `WorksheetGenerator` (Существующий)

**Назначение:** Генератор карточек (A5/A6/A4)

**Может использовать в будущем:**
- 🔄 `useWorksheetGeneration` - рефакторинг логики
- 🔄 `useWorksheetActions` - рефакторинг сохранения
...
1. **Миграция старых генераторов на новые хуки**
   - Рефакторинг `OralWorksheetGenerator`
   - Рефакторинг `WorksheetGenerator`

2. **Расширение FilterBlock**
   - Drag & Drop для изменения порядка блоков
   - Клонирование блоков
   - Сворачивание/разворачивание блоков

3. **Шаблоны контрольных работ**
   - Сохранение структуры блоков отдельно
   - Библиотека готовых шаблонов
   - Импорт/экспорт шаблонов

4. **Улучшение useWorksheetGeneration**
